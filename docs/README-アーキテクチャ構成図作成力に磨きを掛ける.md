...
- ソフトウェアアーキテクチャの基礎
  ...
  - アーキテクトへの期待
    ...
    - アーキテクトの決定は必ず反発される
      - ソフトウェアアーキテクチャを考える ≡ トレードオフとの戦い
        - **できれば不変な構成にしてほしい(平和を保つ)願望とかあるなー**
  ...
  - アーキテクチャ思考
    ...
    - アーキテクトの役割のために幅は広がるが、深さは縮まる
    - アーキテクチャとはGoogleで答えを見つけられないもの
      - **生成AIが活躍する令和の現代でもこの理論は通用するのでしょうか**
      - **トレードオフを分析する行為は、アーキテクトが置かれたビジネス環境に強く依存するとは思うので、引き続きGoogleやAIなどのツールに頼るだけでは答えを見つけにくい側面は残るとは思う**
  - モジュール性の計測
    - 抽象度
      - 抽象と実装の比率
        - インターフェース、抽象クラス = 抽象的なアーティファクト
        - 実装 = 具体的なアーティファクト
    - 不安定度
      - 結合の度合いが高いコードベース＝不安定度が高くなる。変更された時に壊れやすくなる。
    - 主系列からの距離
      - 不安定度と抽象度から求められる、総合的なメトリクス
      - D = | A + I - 1|
      - Aは抽象度、Iは不安定度、どちらも分数、かつ常に0と1の間に収まる（実用的でない極端な抽象度の場合は除く）
      - 距離のメトリクスは、抽象度と不安定度の理想的な関係と仮定できる?
      - 理想的なラインの近くにあるクラスは、２つの競合する関心事が健全に混在していることを示している
        - 距離がでかいと、無駄ゾーンと苦痛ゾーンに突入
          - 無駄に抽象的
          - 逆に具体的すぎて苦痛

    - 結合度
      - グラフ理論に基づいたコードベースの結合度分析
        - Edward Yourdon / Larry Constntine ソフトウェアの構造化設計法
      - 遠心性結合
    ...
    - 凝集度（ぎょうしゅうど）
      - 機能的
        - モジュールが機能するために必要なものが全て含まれている
      - 逐次的
        - 一方の出力をもう一方が入力とする形で、２つのモジュールが相互作用
      - 通信的
        - ２つのモジュールが通信のれんさを形成
        - それぞれの情報を操作、何らかの出力に貢献、データベースにレコードを追加し、その情報に基づいて電子メールを生成
      - 手続き的
        - ２つのモジュールは特定の順序でコードを実行する
      - 時間的
　      - たとえばシステム起動時に初期化しなければならない一見無関係な処理群のように、モジュール間に時間的な依存関係がある場合
      - 論理的
        - 機能的には関連していないが、論理的に関連しているもの
        - 例として紹介されたJavaのStringUtilsクラスは、互いに関連していない静的メソッドの集まりで、論理的な凝集といえる
      - 偶発的
        - 同じソースにあるだけの関係。最も最悪な凝集
  - 設計に関する考慮事項を明らかにする
    - アプリケーションが成功するために必要な運用と設計の基準を明らかにするのがアーキテクチャ
      - 設計の時にアプリケーションが何をすべきかを明らかにするのが要件であることに対する、アーキテクチャの役割
    - 要件をどう実装するかとなぜその方法が選ばれたかに関心を持つ
      - アーキテクチャ特性をドメインの関心事から捉える
        - アーキテクチャの運用特性：可用性、継続性、パフォーマンス、回復性、信頼性／安全性、堅牢性、スケーラビリティ
        - アーキテクチャの構造特性：構成容易性、拡張性、インストール容易性、活用性／再利用性、メンテナンス容易性、可搬性、アップグレード容易性
        - アーキテクチャの横断的特性：アクセシビリティ、長期保存性、認証、認可、合法性、プライバシー、セキュリティ、サポート容易性、ユーザビリティ／達成容易性
